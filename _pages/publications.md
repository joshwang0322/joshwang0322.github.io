---
layout: archive
title: "Publications"
permalink: /publications/
author_profile: true
---

($\alpha-\beta$: alphabetical authorship)


Journal Papers
------
- [**Strongly Efficient Rare-Event Simulation for Regularly Varying Lévy Processes with Infinite Activities**](https://pubsonline.informs.org/doi/abs/10.1287/moor.2024.0627) <br>
  Xingyu Wang, Chang-Han Rhee <br>
  *Mathematics of Operations Research* (ahead of print)

Submitted and Working Papers
------

- [**Large Deviations and Metastability Analysis for Heavy-Tailed Dynamical Systems**](https://arxiv.org/pdf/2307.03479.pdf) <br>
  Xingyu Wang, Chang-Han Rhee <br>
  Major Revision at *Annals of Applied Probability* <br>
  Second Place, **INFORMS George Nicholson Student Paper Competition, 2023**
- [**Tail Asymptotics of Cluster Sizes in Multivariate Heavy-Tailed Hawkes Processes**](https://arxiv.org/pdf/2503.01004) <br>
  ($\alpha-\beta$) Jose Blanchet, Roger J. A. Laeven, Xingyu Wang, Bert Zwart <br>
  Major Revision at *Annals of Applied Probability*
- [**Sample Path Large Deviations for Multivariate Heavy-Tailed Hawkes Processes and Related Lévy Processes**](https://arxiv.org/pdf/2504.01119) <br>
  ($\alpha-\beta$) Jose Blanchet, Roger J. A. Laeven, Xingyu Wang, Bert Zwart <br>
  Major Revision at *Bernoulli*
- [**Global Dynamics of Heavy-Tailed SGDs in Nonconvex Loss Landscape: Characterization and Control**](https://arxiv.org/pdf/2510.20905) <br>
  Xingyu Wang, Chang-Han Rhee <br>
  Submitted to *Operations Research*


   
Conference Papers
------

- [**Multi-agent Multi-armed Bandit with Fully Heavy-tailed Dynamics**](https://www.informs-sim.org/wsc25papers/inv145.pdf) <br>
  Xingyu Wang, Mengfan Xu <br>
  *Winter Simulation Conference, 2025* <br>
- [**Importance Sampling Strategy for Heavy-Tailed Systems with Catastrophe Principle**](https://joshwang0322.github.io/files/WangRhee23b.pdf) <br>
  Xingyu Wang, Chang-Han Rhee <br>
  *Winter Simulation Conference*, 2023
- [**Eliminating Sharp Minima from SGD with Truncated Heavy-tailed Noises**](https://openreview.net/pdf?id=B3Nde6lvab) <br>
  Xingyu Wang, Sewoong Oh, Chang-Han Rhee <br>
  *International Conference on Learning Representations (ICLR)*, 2022  <br>
  **Nemhauser Prize for Best Student Paper, 2020**
- [**Keyword-Based Topic Modeling and Keyword Selection**](https://arxiv.org/pdf/2001.07866.pdf)  <br>
  Xingyu Wang, Lida Zhang, Diego Klabjan <br>
  *IEEE Big Data*, 2021
- [**Rare-Event Simulation for Multiple Jump Events in Regularly Varying Lévy Processes with Infinite Activities**](https://informs-sim.org/wsc20papers/034.pdf)  <br>
  Xingyu Wang, Chang-Han Rhee <br>
  *Winter Simulation Conference*, 2020
- [**Competitive Multi-agent Inverse Reinforcement Learning with Sub-optimal Demonstrations**](http://proceedings.mlr.press/v80/wang18d/wang18d.pdf)  <br>
  Xingyu Wang, Diego Klabjan <br>
  *International Conference on Machine Learning (ICML)*, 2018
